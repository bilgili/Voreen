...
CompactColorPropertyWidget::CompactColorPropertyWidget(ColorProp* prop, QWidget* parent)
...
void CompactColorPropertyWidget::update() {
    currentColor_ = toQColor(property_->get());
    updateColorLabel();
}

void CompactColorPropertyWidget::setProperty() {
    if (!disconnected_) {
        colorDialog();
        updateColorLabel();
...
void CompactColorPropertyWidget::colorDialog() {
    currentColor_.setRgba(QColorDialog::getRgba(currentColor_.rgba()));
}

void CompactColorPropertyWidget::updateColorLabel() {
    colorLbl_->setColor(currentColor_);
}

tgt::Color CompactColorPropertyWidget::toTgtColor(QColor color) {
    return tgt::Color(color.redF(),color.greenF(),color.blueF(), color.alphaF());
}

QColor CompactColorPropertyWidget::toQColor(tgt::Color color) {
