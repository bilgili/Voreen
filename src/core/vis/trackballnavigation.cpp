    ...
    , maxDistance_(maxDist)
{
    if (trackball_) {
        if (defaultBehavior) {
            ...
            trackball_->setMouseRoll(tgt::MouseEvent::MOUSE_BUTTON_RIGHT, 0.5f, tgt::Event::SHIFT);

            trackball_->setMouseWheelZoom(10.f, true, tgt::Event::NONE);
        ...
        // The above is Voreen standard behaviour. If something else is desired, change
...
void TrackballNavigation::resetTrackball() {
    ...
    if (trackball_->getCanvas()->getPainter()) {
        MsgDistr.postMessage(new CameraPtrMsg(VoreenPainter::cameraChanged_, trackball_->getCamera()));
...
/*
        ...
        //FIXME: why? leads to duplication of messages! joerg
        if (trackball_ && trackball_->getCanvas() && trackball_->getCanvas()->getPainter()) {
            std::cout << "send " << msg->id_ << std::endl;
...
*/
        ...
        for (size_t i=0; i < additionalReceivers_.size(); i++)
            additionalReceivers_[i]->processMessage(msg);
/*
