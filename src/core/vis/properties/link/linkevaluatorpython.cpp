...
LinkEvaluatorPython::LinkEvaluatorPython(const std::string& functionName, const std::string& script) {
    // Py_Initialize() may be called several times without harming performance.
    // The documentation states:
    // "This is a no-op when called for a second time
    // (without calling Py_Finalize() first)."
    init(functionName, script);
}
...
bool LinkEvaluatorPython::isActive() {
#ifdef VRN_WITH_PYTHON
    return true;
#else
    return false;
...
void LinkEvaluatorPython::init(const std::string& functionName, const std::string& script) {
    script_ = script;
...
PyObject* cameraToPyCamera(Camera* camera) {
...
TransFuncIntensity* pyTransFuncToTransFunc(PyObject* pyTransFunc) {
    ...
    tf->setTextureDimension(textureWidth, textureHeight);
...
PyObject* transFuncToPyTransFunc(TransFuncIntensity* transFunc) {
    ...
    tgt::Texture* texture = transFunc->getTexture();
...
void LinkEvaluatorPython::deserialize(XmlDeserializer& s) {
    ...
    // Deserialize Python script, forcing unix-style line endings since only these are accepted
    // by the Python interpreter on unix.
    std::string tmp;
    s.deserialize("script", tmp);
    script_ = "";
...
void LinkEvaluatorPython::prepareContext() {
    try {
        PyRun_String(script_.c_str(), Py_file_input, globals, globals);
    } catch (...) {
...
BoxObject LinkEvaluatorPython::eval(const BoxObject& sourceOld, const BoxObject& sourceNew, const BoxObject& targetOld, Property* /*src*/, Property* /*dest*/) {
    ...
    case BoxObject::TRANSFUNC:
        value0 = transFuncToPyTransFunc(sourceOld.getTransFunc());
		checkPythonErrorState();
        value1 = transFuncToPyTransFunc(sourceNew.getTransFunc());
		checkPythonErrorState();
        break;
    ...
    case BoxObject::TRANSFUNC:
        {
            value2 = transFuncToPyTransFunc(targetOld.getTransFunc());
            PyTuple_SetItem(args, 2, value2);
            ret = PyObject_CallObject(function, args);
            checkPythonErrorState();
