...
// transfer functions
uniform TF_SAMPLER_TYPE0 transferFuncCT_;
uniform TF_SAMPLER_TYPE1 transferFuncPET_;
...
vec4 applyCompositing(int curVolume, vec4 result, vec4 color, vec3 samplePos, vec3 gradient, float t, inout float tDepth) {
    if (curVolume == 0) {
        return RC_APPLY_COMPOSITING(result, color, samplePos, gradient, t, tDepth);
    } else {
        return RC_APPLY_COMPOSITING_1(result, color, samplePos, gradient, t, tDepth);
    }
}
    ...
    for (int loop0=0; !finished && loop0<255; loop0++) {
        for (int loop1=0; !finished && loop1<255; loop1++) {
            ...
            // FIXME: entry points need to be replaced by a variable
                ...
                vec4 color0 = RC_APPLY_CLASSIFICATION(transFunc0, voxel0.a);
        ...
        for (int loop0=0; !finished && loop0<255; loop0++) {
            for (int loop1=0; !finished && loop1<255; loop1++) {
                ...
                //if (RC_NOT_MASKED(samplePos0, voxel0.a)) {
                    ...
                    // apply classification
                    vec4 color0 = RC_APPLY_CLASSIFICATION(transFunc0, voxel0.a);
                    vec4 color1 = RC_APPLY_CLASSIFICATION(transFunc1, voxel1.a);
                    ...
                    // if opacity greater zero, apply compositing
                    ///if (color0.a > 0.0)
                    ///    result0 = RC_APPLY_COMPOSITING(result0, color0, samplePos0, voxel0.xyz, t0, tDepth);
                    ///if (color1.a > 0.0)
                    ///    result1 = RC_APPLY_COMPOSITING(result1, color1, samplePos1, voxel1.xyz, t1, tDepth);
                    ...
                    // FIXME: !!!!
                    result = RC_APPLY_COMPOSITING(result, colorSum, samplePos1, voxel1.xyz, t1, tDepth);

                    //result = RC_APPLY_COMPOSITING(result, color1, samplePos1, voxel1.xyz, t1, tDepth);
                    //result = RC_APPLY_COMPOSITING(result, color0, samplePos0, voxel0.xyz, t0, tDepth);
                //}
                //finished = earlyRayTermination(result.a, 0.9);
