...
const std::string GeometrySource::getProcessorInfo() const {
    return std::string("<p>Loads a point list or segmented point list from a text file. Each point is expected to    \
                        consists of three components that are separated by white space. </p>                               \
                        ...
                        <p>Additionally, the number of items that are to be skipped after each point can be specified.</p>");
}
