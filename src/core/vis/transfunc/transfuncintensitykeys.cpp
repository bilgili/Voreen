...
const std::string TransFuncIntensityKeys::loggerCat_("voreen.voreen.TransFunc.IntensityKeys");

TransFuncIntensityKeys::TransFuncIntensityKeys(int width/*=256*/)
  : TransFuncIntensity() {
	tex_ = 0;
    createTex(width);
    createStdFunc();
...
TransFuncIntensityKeys::TransFuncIntensityKeys(const TransFuncIntensityKeys& tf) : TransFuncIntensity() {
...
TransFuncIntensityKeys::~TransFuncIntensityKeys() {
    for (size_t i = 0 ; i < keys_.size() ; ++i)
        delete keys_[i];
}

void TransFuncIntensityKeys::createTex(int width) {
    if (tex_)
        delete tex_;
...
void TransFuncIntensityKeys::createStdFunc() {
    clearKeys();
...
void TransFuncIntensityKeys::createAlphaFunc(){
    clearKeys();
...
tgt::vec4 TransFuncIntensityKeys::getMeanValue(float segStart, float segEnd) const {
    tgt::ivec4 result(0);
...
col4 TransFuncIntensityKeys::getMappingForValue(float value) const {
...
void TransFuncIntensityKeys::updateTexture() {
/*    if (!tex_)
        createTex(256);
    int imageWidth = tex_->getWidth();

    for (int x = 0 ; x < imageWidth ; ++x)
        tex_->texel<col4>(x) = col4( getMappingForValue( (float)x / imageWidth) );
    tex_->uploadTexture();
*/
    if (!tex_)
        createTex(256);
...
void TransFuncIntensityKeys::updateTexture(int leftValue, int rightValue) {
    if (!tex_)
        createTex(256);
    
    for (int x = leftValue ; x < rightValue ; ++x)
        tex_->texel<col4>(x) = col4( getMappingForValue( (float)x / rightValue) );
    
    tex_->uploadTexture();
}

const int TransFuncIntensityKeys::getNumKeys() const{
    return keys_.size();
}

TransFuncMappingKey* TransFuncIntensityKeys::getKey(int i) const{
    return keys_.at(i);
}

const std::vector<TransFuncMappingKey*> TransFuncIntensityKeys::getKeys() const{
    return keys_;
}

void TransFuncIntensityKeys::addKey(TransFuncMappingKey* key) {
...
void TransFuncIntensityKeys::updateKey(TransFuncMappingKey* /*key*/) {
    std::sort(keys_.begin(), keys_.end(), sortFunction);
}

void TransFuncIntensityKeys::removeKey(TransFuncMappingKey* key) {
...
void TransFuncIntensityKeys::clearKeys() {
    std::vector<TransFuncMappingKey *>::iterator keyIterator = keys_.begin();
...
bool TransFuncIntensityKeys::isSignificant() const {
...
bool TransFuncIntensityKeys::isEmpty() const {
    return keys_.empty();
}

bool TransFuncIntensityKeys::save(const std::string& filename) {
    TiXmlDocument doc;
    TiXmlDeclaration* decl = new TiXmlDeclaration( "1.0", "", "" );
    doc.LinkEndChild( decl );

    TiXmlElement* root = new TiXmlElement("TransFuncIntensityKeys");
...
bool TransFuncIntensityKeys::savePNG(const std::string& filename, int width) {
    ...
    for (int x = 0 ; x < width ; ++x) {
        tgt::col4 c = getMappingForValue( (float)x/width );
        im[(x*4)+0] = c.r;
        im[(x*4)+1] = c.g;
        im[(x*4)+2] = c.b;
        im[(x*4)+3] = c.a;
...
bool TransFuncIntensityKeys::load(const std::string& filename) {
...
#ifdef VRN_WITH_DEVIL
bool TransFuncIntensityKeys::loadWithDevIL(const std::string& filename) {
    ILuint imageID;
    ILboolean success;
    ...
    generateKeys((char*)newImageData , imageWidth);
...
bool TransFuncIntensityKeys::loadXML(const std::string& filename) {
...
bool TransFuncIntensityKeys::loadTextTable(const std::string& filename) {
    if (filename == "") {
        createStdFunc();
        return false;
    }
...
bool TransFuncIntensityKeys::loadOsirixCLUT(const std::string& filename) {
...
bool TransFuncIntensityKeys::loadImageJ(const std::string& filename) {
...
int TransFuncIntensityKeys::openImageJBinary(std::ifstream& fileStream, bool raw) {
...
int TransFuncIntensityKeys::openImageJText(std::ifstream& fileStream){
...
void TransFuncIntensityKeys::generateKeys(char* data, int width){
    ...
    for (int iter = 2 ; iter < width ; ++iter){
        ...
        if (differenceQuotientChanged) {
            if (differenceQuotientNotZero) {
                ...
                TransFuncMappingKey* newkey = new TransFuncMappingKey( iter/(float)(width-1) ,
                ...
                addKey(
                    new TransFuncMappingKey((iter - 1)/(float)(width-1),
                    tgt::col4( data[4*(iter-1) + 0], data[4*(iter-1) + 1], data[4*(iter-1) + 2], data[4*(iter-1) + 3] )
                    ));
            }
            else {
                // Just add a key
                addKey(
                    new TransFuncMappingKey( iter/(float)(width-1),
...
inline int TransFuncIntensityKeys::readInt(std::ifstream& stream) {
...
inline short TransFuncIntensityKeys::readShort(std::ifstream& stream) {
...
inline double TransFuncIntensityKeys::readDouble(std::ifstream& stream) {
