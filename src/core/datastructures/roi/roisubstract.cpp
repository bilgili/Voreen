...
ROISubstract::ROISubstract(Grid grid) : ROIAggregation(grid) {
}

bool ROISubstract::inROI(tgt::vec3 p) const {
    bool in = false;
    if(children_.size() >= 1)
...
std::vector<const ControlPoint*> ROISubstract::getControlPoints() const {
    //TODO
    return std::vector<const ControlPoint*>();
}

std::vector<const ControlPoint*> ROISubstract::getControlPoints(tgt::plane /*pl*/) const {
    //TODO
    return std::vector<const ControlPoint*>();
}
