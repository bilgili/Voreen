...
class SVD 
{
  ...
  public:
   ...
   SVD (const Array2D<Real> &Arg) {
      ...
      if (wantu) {
         ...
         for (k = nct-1; k >= 0; k--) {
            if (s[k] != 0.0) {
               ...
               U[k][k] = static_cast<Real>(1.0) + U[k][k];
               for (i = 0; i < k-1; i++) {
                  U[i][k] = static_cast<Real>(0.0);
               }
            } else {
               for (i = 0; i < m; i++) {
                  U[i][k] = static_cast<Real>(0.0);
               }
               U[k][k] = static_cast<Real>(1.0);
            }
         }
      }
      ...
      int pp = p-1;
      int iter = 0;
      Real eps(static_cast<Real>(pow(2.0,-52.0)));
      while (p > 0) {
         int k=0;
		 int kase=0;
         ...
         if (k == p-2) {
            ...
            for (ks = p-1; ks >= k; ks--) {
               ...
               Real t( static_cast<Real>((ks != p ? abs(e[ks]) : 0.0)) + 
                          static_cast<Real>((ks != k+1 ? abs(e[ks-1]) : 0.0)));
               if (abs(s[ks]) <= eps*t)  {
                  s[ks] = 0.0;
                  break;
               }
         ...
         switch (kase) {
            ...
            case 3: {
               ...
               Real sk = s[k]/scale;
               Real ek = e[k]/scale;
               Real b = ((spm1 + sp)*(spm1 - sp) + epm1*epm1)/static_cast<Real>(2.0);
            ...
            case 4: {
               ...
               if (s[k] <= 0.0) {
                  s[k] = static_cast<Real>((s[k] < 0.0 ? -s[k] : 0.0));
                  if (wantv) {
                     for (i = 0; i <= pp; i++) {
                        V[i][k] = -V[i][k];
                     }
