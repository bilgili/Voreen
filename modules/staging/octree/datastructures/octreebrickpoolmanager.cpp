...
const uint16_t* OctreeBrickPoolManagerRAM::getBrick(uint64_t virtualMemoryAddress) const
...
uint16_t* OctreeBrickPoolManagerRAM::getWritableBrick(uint64_t virtualMemoryAddress) const
    throw (VoreenException)
{
    return const_cast<uint16_t*>(getBrick(virtualMemoryAddress));
}

void OctreeBrickPoolManagerRAM::setBrickUnused(uint64_t virtualMemoryAddress) const {
    //TODO
}
...
uint64_t OctreeBrickPoolManagerRAM::getBrickPoolMemoryAllocated() const {
    return getNumBrickBuffers() * getBrickBufferSizeInBytes();
}
...
void OctreeBrickPoolManagerRAM::serialize(XmlSerializer& s) const {
    ...
    for (size_t i=0; i<brickBuffers_.size(); i++) {
        const std::string bufferFile = octreePath + "/brickbuffer" + itos(i, 6, '0') + ".raw";
...
void OctreeBrickPoolManagerRAM::deserialize(XmlDeserializer& s) {
    ...
    for (int i=0; i<numBuffers; i++) {
        ...
        const std::string bufferFile = tgt::FileSystem::cleanupPath(octreePath + "/brickbuffer" + itos(i, 6, '0') + ".raw");
