...
RptTCTTooltip::RptTCTTooltip ( QGraphicsItem * parent /* = 0*/) // compiliert sonst nicht (Dirk)
    : QGraphicsRectItem(parent), id_(0), tc_(0), image_(0)
{}

RptTCTTooltip::RptTCTTooltip ( const QRectF & rect, QGraphicsItem * parent /* = 0*/)
    : QGraphicsRectItem( rect, parent), id_(0), tc_(0), image_(0)
{}

RptTCTTooltip::RptTCTTooltip( qreal x, qreal y, qreal width, qreal height, QGraphicsItem * parent /* = 0*/) 
    : QGraphicsRectItem( x, y, width, height, parent), id_(0), tc_(0), image_(0)
{}
...
void RptTCTTooltip::initImage()
{
    ...
    delete[] temp;
    // fit rect
    float image_aspect = (float)size.x/(float)size.y;
