...
bool RptGuiItem::disconnectAll() {
    ...
    /* WTF WTF WTF - d_kirs04
    std::vector<RptPropertySetItem*> propTmp = propertySets_;
    for (size_t i=0; i < propTmp.size(); i++) {
        if( propTmp[i] != 0 )
            propTmp[i]->disconnectGuiItem(this);
    }
    for (size_t i = 0; i < propertySets_.size(); i++)
        propertySets_.erase(propertySets_.begin() + i);
    */
    // my turn... let's do this nicely
