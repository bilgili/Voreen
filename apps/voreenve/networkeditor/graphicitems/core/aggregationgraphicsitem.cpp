...
AggregationGraphicsItem::AggregationGraphicsItem(QList<PortOwnerGraphicsItem*> items, voreen::NetworkEditor* networkEditor)
    : PortOwnerGraphicsItem(0,networkEditor)
...
QRectF AggregationGraphicsItem::boundingRect() const {
    return currentStyle()->AggregationGI_boundingRect(this);
}

QPainterPath AggregationGraphicsItem::shape() const {
    return currentStyle()->AggregationGI_shape(this);
}

void AggregationGraphicsItem::initializePaintSettings() {
    currentStyle()->AggregationGI_initializePaintSettings(this);
}

void AggregationGraphicsItem::prePaint(QPainter* painter, const QStyleOptionGraphicsItem* option) {}

void AggregationGraphicsItem::mainPaint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget) {
    currentStyle()->AggregationGI_paint(this,painter,option,widget);
}
