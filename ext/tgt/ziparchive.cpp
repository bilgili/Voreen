...
size_t ZipArchive::deflateToDisk(File& inFile, std::ofstream& archive, unsigned long& crc) {
    ...
    do {
        ...
        crc = crc32(crc, reinterpret_cast<Bytef*>(inBuffer), read);
        readTotal += read;
        strm.avail_in = read;
        ...
        const size_t uncompressedSize, size_t archiveOffset)
{
...
#else
    size_t ZipArchive::deflateToDisk(File&, std::ofstream&, unsigned long&) {
        LERROR("TGT was not compiled with zlib!");
        return 0;
    }

    File* ZipArchive::inflateToDisk(const std::string&, const size_t, 
        const size_t, size_t)
    {
        LERROR("TGT was not compiled with zlib!");
        return 0;
    ...
    File* ZipArchive::inflateToMemory(const std::string&, size_t,
        const size_t, size_t)
    {
        LERROR("TGT was not compiled with zlib!");
        return 0;
...
bool ZipArchive::readEOCDHeaderRecord(ZipArchive::ZipEOCDHeaderRecord& eocdHeaderRec)
{
    ...
    //
    int fileSize = static_cast<int>(archive_->size());
    int maxBufferSize = static_cast<size_t>(0xFFFF) + SIZE_ZIPEOCDRECORD;
    int bufferSize = (maxBufferSize < fileSize) ? maxBufferSize : fileSize;
    char* buffer = new char[bufferSize];
                                       ...
                                       const std::vector<ZipArchive::ArchivedFile>& files)
{
    ...
    eocd.numberOfEntriesInThisCD = files.size();
    eocd.numberOfEntriesInCD = files.size();
                                 ...
                                 std::vector<ZipArchive::ArchivedFile>& newFiles)
{
    ...
    for (size_t i = 0; i < newFiles.size(); ++i) {
        ...
        if (error == true) {
            ...
            fileHeader.filenameLength = newFiles[i].fileName_.size();
            fileHeader.extraFieldLength = newFiles[i].fileExtra_.size();
            fileHeader.fileCommentLength = newFiles[i].fileComment_.size();
            fileHeader.diskNumberStart = 0;
            fileHeader.internalFileAttributes = 0;
            fileHeader.externalFileAttributes = 0;
            fileHeader.localHeaderOffset = newFiles[i].localHeaderOffset_;
        }
        ++counter;
    }   // for (i
    return counter;
