...
RegularFile::RegularFile(const std::string& filename) : File(filename)
{
...
MemoryFile::MemoryFile(char* data, size_t size, const std::string& filename, bool deleteData) : File(filename)
{
    data_ = data;
    size_ = size;
    pos_ = 0;
...
TarFileFactory::TarFileFactory(const std::string& filename, const std::string& rootpath)
{
    ...
    while(!end)
    {
        ...
        for(int i = 0; i < 512; i++)
        {
            if(currecord->charptr[i] != 0) allzero = false;
        }
        ...
        if(!allzero)
        {
            ...
            if(filesize > 0)
            {
				...
				LDEBUG("  Add it as " << rootpath+currecord->header.name << "; filesize: " << filesize << "; offset " << file->tell());
...
File* TarFileFactory::open(const std::string& filename)
{
    ...
    TarFile* tf = new TarFile(filename, filename_, files_[filename].offset_, files_[filename].size_);
    return tf;
}
...
ZipFileFactory::ZipFileFactory(const std::string& filename, const std::string& rootpath)
{
    ...
    if(*(unsigned int*) eocheader->header.signature != 0x06054b50)
    {
    ...
    unsigned int startOfCD = *(unsigned int*) eocheader->header.offsetStartCD;
    LDEBUG("Start offset of CD: " << startOfCD);

    file_->seek(startOfCD, File::begin);
    unsigned short int entries = *(unsigned short int*)eocheader->header.numberOfEntriesInThisCD;
    ...
    for(int i=0; i<entries; i++)
    {
...
#ifndef tgtNOZIPCHECKS
        if(*(unsigned int*) h->header.signature != 0x02014b50)
        {
        ...
        switch(*compMeth)
        {
            case 0: LDEBUG("The file is stored (no compression)");
                    files_[rootpath+fname].offset_ = *(unsigned int*)h->header.localHeaderOffset;
                    LDEBUG("Added file " << fname << "@" << files_[rootpath+fname].offset_);
                    break;
            case 1: LWARNING("The file is Shrunk");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            case 2: LWARNING("The file is Reduced with compression factor 1");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            case 3: LWARNING("The file is Reduced with compression factor 2");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            case 4: LWARNING("The file is Reduced with compression factor 3");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            case 5: LWARNING("The file is Reduced with compression factor 4");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            case 6: LWARNING("The file is Imploded");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            case 7: LWARNING("Reserved for Tokenizing compression algorithm");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            case 8: LDEBUG("The file is Deflated");
                    #ifdef TGT_HAS_ZLIB
                        files_[rootpath+fname].offset_ = *(unsigned int*)h->header.localHeaderOffset;
                        LDEBUG("Added file " << fname << "@" << files_[rootpath+fname].offset_);
                    #else
                        LWARNING("tgt was compiled without zlib support, file cannot be read and will not be added to filesystem!");
                    #endif
                    break;
            case 9: LWARNING("Enhanced Deflating using Deflate64(tm)");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            case 10: LWARNING("PKWARE Date Compression Library Imploding");
                    LWARNING("Compression method unsupported, file will not be added to filesystem!");
                    break;
            default: LWARNING("Compression method unsupported, file will not be added to filesystem!");
        }
...
File* ZipFileFactory::open(const std::string& filename)
{
    ...
    //file_->seek(files_[filename].offset_, File::begin);
...
#ifndef tgtNOZIPCHECKS
    LDEBUG("FileHeader:");
    if(*(unsigned int*) currecord->header.signature != 0x04034b50)
    {
        LERROR("Local signature does not match!");
        delete currecord;
        return 0;
    }
    LDEBUG("Version needed: " << ((int)currecord->header.versionNeeded[0])/10 << "." << ((int)currecord->header.versionNeeded[0])%10);
#endif
    ...
    switch(*compMeth)
    {
        case 0: LDEBUG("The file is stored (no compression)");
                break;
        case 1: LERROR("The file is Shrunk");
                delete currecord;
                return 0;
        case 2: LERROR("The file is Reduced with compression factor 1");
                delete currecord;
                return 0;
        case 3: LERROR("The file is Reduced with compression factor 2");
                delete currecord;
                return 0;
        case 4: LERROR("The file is Reduced with compression factor 3");
                delete currecord;
                return 0;
        case 5: LERROR("The file is Reduced with compression factor 4");
                delete currecord;
                return 0;
        case 6: LERROR("The file is Imploded");
                delete currecord;
                return 0;
        case 7: LERROR("Reserved for Tokenizing compression algorithm");
                delete currecord;
                return 0;
        case 8: LDEBUG("The file is Deflated");
                break;
        case 9: LERROR("Enhanced Deflating using Deflate64(tm)");
                delete currecord;
                return 0;
        case 10: LERROR("PKWARE Date Compression Library Imploding");
                delete currecord;
                return 0;
        default: LERROR("Unknown compression method (>10)");
                delete currecord;
                return 0;
    }
    ...
    if(*compMeth==8)
    {
        ...
        {
            case Z_OK: LDEBUG("success");
                break;
            case Z_MEM_ERROR: LDEBUG("there was not enough memory");
                break;
            case Z_BUF_ERROR: LDEBUG("there was not enough room in the output buffer");
                break;
            case Z_DATA_ERROR: LDEBUG("DATA ERROR");
                break;
            default: LDEBUG("DEFAULT");
        }
