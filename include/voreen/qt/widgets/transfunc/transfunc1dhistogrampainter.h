 ...
 */
class TransFunc1DHistogramPainter : public tgt::Painter {
public:
    /**
     * @param canvas canvas that belongs to this painter
     */
    TransFunc1DHistogramPainter(tgt::GLCanvas* canvas);

    ~TransFunc1DHistogramPainter();
    ...
    /// Returns the left side of the canvas in real-world coordinates
    float getViewLeft() const;
    float getViewRight() const;

protected:
    TransFunc* tf_; ///< the displayed transfer function
    const VolumeHistogramIntensity* histogram_; ///< the displayed transfer function
...
#endif // VRN_TRANSFUNC1DKEYSHISTOGRAMPAINTER_H
